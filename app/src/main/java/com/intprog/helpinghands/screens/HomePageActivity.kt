...
class HomePageActivity : AppCompatActivity() {
    ...
    private fun navigateToStatusPage(campaign: Campaign) {
        ...
        when (campaign.type) {
            CampaignType.VOLUNTEER -> {
                val volunteerDetailsList = getVolunteerCampaignDetails(campaign.title)
                if (volunteerDetailsList != null) {
                    for (volunteerDetails in volunteerDetailsList) {
            ...
            CampaignType.DONATION -> {
                val donationDetailsList = getDonationCampaignDetails(campaign.title)
                if (donationDetailsList != null) {
                    for (donationDetails in donationDetailsList) {
            ...
            CampaignType.UNSPECIALIZED -> {
                val unspecializedDetailsList = getUnspecializedActivityDetails(campaign.title)
                if (unspecializedDetailsList != null) {
                    for (unspecializedDetails in unspecializedDetailsList) {
    ...
    private fun getVolunteerCampaignDetails(title: String): List<VolunteerCampaignPost>? {
        val sharedPrefs = getSharedPreferences("VolunteerCampaignPrefs", Context.MODE_PRIVATE)
        val json = sharedPrefs.getString("posts", null) // Use "posts" as the key
        if (json == null) {
            Log.d("HomePageActivity", "No details found for title: $title")
            return null
        }
        return Gson().fromJson(json, object : TypeToken<List<VolunteerCampaignPost>>() {}.type)
    }

    private fun getDonationCampaignDetails(title: String): List<DonationCampaignPost>? {
        ...
        return Gson().fromJson(json, object : TypeToken<List<DonationCampaignPost>>() {}.type)
    }

    private fun getUnspecializedActivityDetails(title: String): List<UnspecializedActivityPost>? {
        ...
        return Gson().fromJson(json, object : TypeToken<List<UnspecializedActivityPost>>() {}.type)
    }
}
